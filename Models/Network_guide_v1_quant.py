...
class Encoder(nn.Module):
    ...
    def forward(self, inputs_Y, inputs_UV):
        ...
        Y_split = self.squeeze(inputs_Y, 2)
        UV_out = self._model_UV(inputs_UV)
        U_split = inputs_UV[:,0:1,:,:]
        V_split = inputs_UV[:,1:2,:,:]
        att_score_UV = Y_split * U_split + Y_split * V_split
        att_score_UV = torch.sum(att_score_UV, dim = (2, 3, 4))
        att_para = torch.softmax(att_score_UV, dim = 0)
...
class quant_context(nn.Module):
    def __init__(self, context):
        super(quant_context, self).__init__()
        self.context = context
        self.quant = quant.QuantStub()
        self.dequant = quant.DeQuantStub()
    def forward(self, input):
        input = self.quant(input)
        output = self.context(input)
        output = self.dequant(output)
        return output


class Network_guide_v1_quant(nn.Module):
    ...
    def forward(self, y_comp, u_comp, v_comp):
        ...
        y, att_para = self.encoder(y_comp2, uv_comp2)
